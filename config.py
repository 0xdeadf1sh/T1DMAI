...
D_MODEL = 256                    # Hidden dimension throughout the transformer
N_LAYERS = 16                     # Number of transformer blocks
N_HEADS = 16                      # Number of temporal attention heads
...
# Input features per timestep (`N_INPUT_FEATURES = 3`):
...
# space as its OUTPUT. carb / insulin keep their log1p + z encoding. There are NO
# mask bits and NO temporal (sin/cos hour-of-day, day-of-week) features. The model
# is ALWAYS conditioned on the future carb / insulin: in the prediction zone carb
# (feat 1) and insulin (feat 2) carry their true (training / metrics) or announced
# (what-if / deployment) values, while bg (feat 0) is always zeroed there (it is
# what the model predicts).
N_INPUT_FEATURES = 3             # Input features per timestep: bg_absolute (risk-space), carbs, insulin
PATCH_DIM = PATCH_SIZE * N_INPUT_FEATURES  # PATCH_SIZE * N_INPUT_FEATURES (no mask bits, no temporal feats)

# Pred-zone signal handling (output-channel space {0:carb, 1:insulin}).
NON_MASKABLE_FEATS = (0,)        # bg_absolute input slot is always zeroed in the pred zone
MASKABLE_FEATS = (1, 2)          # carb / insulin input feats carry the announced future doses
# Single shared (output-channel -> input-feat) mapping consumed by BOTH data.py
# (pred-zone dose write) and inference.py (override-write). The only place the
# output-channel -> feature +offset lives.
CHANNEL_TO_FEAT = {0: 1, 1: 2}   # carb -> feat 1; insulin -> feat 2

# === Sequence Lengths ===
# Variable 8–24 h context. The T1DMSIM diff report (§0.5) measures pooled-CGM
# Pearson ACF₀.₂ at 7.7 h for the simulator and 2.4–4.8 h across the three
# real cohorts, so anything past ~8–12 h adds little autoregressive signal —
# 8 h is the §0.5-recommended floor where most of the available autoregressive
# information is still in scope. 24 h is held as the working ceiling for two
# reasons: (1) per-patient basal cadence is 18–30 h, so one full basal cycle
# is mostly visible in context, supporting patient-identity inference for the
...
# samples n_ctx uniformly in [MIN, MAX]; the collate function left-pads to
# MAX_CONTEXT_PATCHES so the prediction horizon stays at the right edge.
...
# === Prediction Horizon ===
# The model predicts a contiguous window of length ``PREDICTION_HORIZON_HOURS``
# that may start at any patch-aligned position in the 24-hour day. A single
...
# vs night dynamics from the bg / carb / insulin context alone, without an
# external band restriction.
...
# high-capacity head off the prediction-zone hidden state. The head emits, per
# (patch, timestep), a median delta plus 2*N_SPREADS softplus spreads from which
# the 7 quantiles are assembled monotonically around an anchor of
# f(last_bg) (so median == f(persistence) at init). See utils.assemble_quantiles.
...
#     (median == f(last_bg)), departing from there under the loss.
...
BG_HEAD_INIT_SCALE = 1e-2        # final-layer init std — small ⇒ median≈f(last_bg) at init ⇒ forecast starts at persistence
...
# The head no longer emits PATCH_SIZE independent values per (patch, channel);
# it emits BG_HEAD_STEP_BASIS_DIM (= K) coefficients that are expanded across the
# PATCH_SIZE within-patch timesteps by a FIXED low-frequency basis (model.py
# step_basis buffer). With K < PATCH_SIZE the highest-frequency (period-2) mode
# is unrepresentable, so an intra-patch zigzag of the median cannot be emitted by
# construction. K = PATCH_SIZE recovers the old fully-free per-step head exactly.
...
# Median-assembly mode (R3 selector, replaces the old BG_HEAD_CUMULATIVE_MEDIAN
# boolean). assemble_quantiles is the SOLE chokepoint (train forward, inference.predict,
# predict_rolling), so this gate propagates everywhere identically.
#   * 'global'      — R3 DEFAULT. The per-patch median delta head_raw[...,0] is
#                     projected onto a fixed low-frequency DCT-II subspace span(Bg)
#                     over the FULL P*S prediction horizon (Bg orthonormal, G columns):
#                     m = anchor + (delta_flat @ Bg @ Bgᵀ).reshape(P,S). A projection is
#                     an L2 CONTRACTION (||proj(x)|| <= ||x||) so the per-patch offset
#                     o[p]=m[:,p,0]-anchor CANNOT drift or amplify — it is bounded and
#                     non-monotone in p, structurally killing R1's unconstrained-
#                     integrator drift. Low-pass G keeps the median smooth (the per-patch
#                     period-2 / seam sawtooth is unrepresentable), so the seam penalty
#                     is redundant under this mode. C0 seam-continuity and first-step
#                     pinning are NOT guaranteed (intentional — enforcing exact C0 is what
#                     caused R1's drift).
...
# G = number of low-frequency DCT-II basis columns over the full P*S horizon
# (1 <= G <= P*S). Small G (BG_HEAD_MEDIAN_GLOBAL_DIM over the
# PREDICTION_PATCHES*PATCH_SIZE horizon) keeps the median in a smooth low-frequency
# subspace: periods below ~2*(P*S)/G steps (incl. the per-patch seam sawtooth) are
# filtered out, while col0 (the DC mode) retains the persistence level. The basis kind
...
# An auxiliary head that classifies each prediction PATCH's absolute hour-of-day
# into one of TIME_PROBE_N_BINS circular bins (softmax logits, NO mean-pool — the
# head runs on every prediction-patch hidden state). It is trained by cross-entropy
# against a wrapped-Gaussian circular soft label (TIME_PROBE_LABEL_SMOOTH_BINS std in
# bins). The probe is trained by PER-PATCH CROSS-ENTROPY ONLY: within one 2 h window the
# P patches share one forward/context and are already clock-consistent through the trunk
# (ablation: tod_jump_h ≈ 0.12 h with or without a within-window penalty), so the old
# within-window inter-patch advance penalty is removed. A SEPARATE cross-window
# teacher-forced penalty (TIME_PROBE_CROSS_WINDOW_WEIGHT) couples consecutive
# INDEPENDENT-forward 2 h windows so the rolling clock advances by exactly one horizon
...
TIME_PROBE_N_BINS = max(1, round(24.0 / PREDICTION_HORIZON_HOURS))   # 12 two-hour bins at 2 h horizon
...
# 399/41 clamp rails are discarded) plus optional hypoglycemia oversampling
...
# ALiBi slopes), none of which is a matrix-followed-by-a-norm. Since
# gamma_max = peak LR and gamma_t = peak*ratio, the correction factor is exactly
# the schedule ``ratio``: the per-step multiply p*(1 - gamma_t*lambda) becomes
...
# === Loss ===
SIMULATOR_WARMUP_HOURS = 48.0    # Hours discarded from the start of every simulator run. Empirically (see scratch warmup probe) the simulator's basal pipeline starts at zero — first basal/bolus events land 2–7 h in — and basal does not reach Bateman-PK steady state until ~36–48 h given the 18–30 h per-patient cadence. 24 h leaves the early trajectory in a cold-start ramp where mean BG climbs to ~300 mg/dL before the basal catches up. 48 h puts hour 0 of the trajectory at proper steady state.
...
# 1-homogeneous in (cost, γ) so the single-cell cost peak (= 40.0 = (f(400)-f(40))²)
# does not force any particular γ. γ purely tunes how soft the min is.
...
# === Checkpoint schema-version stamps (resume-refusal / hard-reset) ===
# Authoritative source for the architecture + loss-schema strings stamped into
# every checkpoint. train.py prefers these over its local fallback sentinels, so
# config is the single source of truth. Bump on any arch/loss change that must
# refuse a pre-redesign --resume rather than mis-map optimizer/EMA state.
ARCH_VERSION = 'risk-v3'              # model-architecture schema (risk-space input + output; [40,400]-anchored Kovatchev, no input/target smoothing)
LOSS_SCHEMA = 'kendall-pinball-dilate-v3'  # loss schema (Kendall-Gal dynamic pinball/DILATE uncertainty-weighted combine on the risk-v3 arch)

# === Input / target signal space (NO smoothing) ===
# The model consumes the RAW simulator post-noise signals (bg_observed with CGM
# sensor noise, carb/insulin with the AR(1) absorption noise) directly — there is
# no causal smoother on inputs or on the forecast target. The forecast TARGET and
# the last_bg anchor are the raw bg_observed as well, so inputs, target, loss and
# metrics all live in one raw post-noise space (bg is only clamped to the physical
# [BG_CLAMP_MIN, BG_CLAMP_MAX] range; carb/insulin are floored at 0 by the log1p
# transform). Deployment realism is intrinsic: the live CGM/dose stream is fed
# exactly as-is, and the autoregressive roll re-feeds the model's own raw output.
...
# The model is always conditioned on the announced future carb/insulin, so the
# headline property is that it responds correctly to HYPOTHETICAL doses the
# patient dials in. The counterfactual probe (train._run_counterfactual_probe)
# perturbs the prediction-zone doses relative to a baseline forecast and checks
# the BG response has the correct sign, is monotone, and resolves excursions.
# Magnitudes are in RAW units (grams of carb, units of insulin), injected as a
# bolus at the first prediction patch then re-normalized through the log1p stats.
...
# space; carb/insulin keep their log1p+z transform. A single unconditional stats
# file holds all three channels (see normalization.RISK_SPACE_CHANNELS).
