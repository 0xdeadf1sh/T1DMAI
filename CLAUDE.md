...
- **Context window is variable 84–168 h** (`MIN_CONTEXT_PATCHES = 168`, `MAX_CONTEXT_PATCHES = 336` at 2 patches/hour, since `PATCH_SIZE = 6` makes one patch 30 min). Each training sample draws `n_ctx` uniformly in `[MIN, MAX]`; `collate_fn` left-pads to the batch maximum. The simulator's own ACF analysis in `T1DMSIM/diff/README.md` §0.5 measures pooled-CGM ACF₀.₂ at 5.3 h, so the `MIN` sits far above the autocorrelation rather than being derived from it. The floor spans several full basal dose cycles — both analogues are injected on a fixed 24 h interval with no jitter (they differ in *action* duration, glargine 26 h / degludec 42 h, not in dosing) — and preserves GT context through the 8 h night long-horizon rolling validation (`inference.predict_rolling` slides the window once it exceeds `MAX_CONTEXT_PATCHES`). At inference time the model accepts any `n_ctx ≥ MIN_CONTEXT_PATCHES`.
...
- **Loss = `risk_loss.risk_total_loss(q_tau, median, true_bg_mgdl, weighting, valid, mask_idx)`** (soft-DTW DP in `dilate.py`). The target is f-transformed exactly once, at the top: `y_risk = kovatchev_f_target(true_bg_mgdl)`, shared by every term. `valid` and `mask_idx` default to `None`, which means the dense right-edge case — pass them on every real call.
  ...
  - **`L_M` (MSE on the median)**: `mean over valid (slot, step) of (median − y_risk)²`, pinball's denominator rule. It shares DILATE's Kendall-Gal slot: `L_DR = (1 − MSE_ALPHA)·L_D + MSE_ALPHA·L_M`. `MSE_ALPHA = 0` skips the MSE; `= 1` skips the soft-DTW and logs `loss_D` and every `loss_D_L{L}` as 0.
  - **Kendall-Gal combine** (numerically safe form): `L_KG = 0.5·exp(−2·log_sigma_Q)·L_Q + log_sigma_Q + 0.5·exp(−2·log_sigma_D)·L_DR + log_sigma_D`. `log_sigma_Q`/`log_sigma_D = nn.Parameter(zeros(()))` (init `KENDALL_LOGVAR_INIT`), clamped `[-7, 7]`, ndim==0 ⇒ AdamW group (never Muon), EMA-excluded. `L_KG` is the full training total — there is no smoothness, curvature or seam term.
...
- **Rolling prediction BG fill.** `predict_rolling()` is **BG-autoregressive only**: `median → f_inv → mg/dL → normalize → bg_absolute slot 0`. carb (feat 1) / insulin (feat 2) / exercise (feat 3) come from `overrides_fn` if supplied, else from the **zero-RAW normalized baseline** — `normalize(0.0)` per channel, NOT `torch.zeros`, whose `z = 0` is a phantom event that corrupts a no-dose roll. The caller MUST pass `normalization_stats`: `predict_rolling` raises `ValueError` on `None` rather than falling back to the CWD file.
  ...
  - **The rolling band carry is `predict_rolling`'s own, not `assemble_quantiles`'s.** Each roll measures its NATIVE terminal-step offsets — SIX of them, one per level, `q_tau[-1,-1,med+1+k] − m` and `m − q_tau[-1,-1,med-1-k]` — on the PRE-carry fan; re-opens that roll's fan to `hypot(carry_k, native_offset_k)` per level about the untouched median; derives `bands = f_inv(q_tau)` from the widened fan, with conformal on top of that; and only THEN folds the native offsets into the carry, also in quadrature. So roll 0 is unwidened and each level's carry grows like `√n` over `n` rolls. Two ways to get this wrong, both of which have been in the tree: ONE carry shared by the levels re-seeds every level from the outermost one's accumulation, so the next roll's .75 edge opens outside this roll's .95 edge and the fan flattens into a slab; and ADDING the carry rather than composing it in quadrature is the perfectly-correlated bound, twice too wide by the fourth roll and pinned to the physiological rails by the eighth. Note that `hypot(carry_k, native_k)` IS the terminal offset of the fan just drawn, which is what `T1DMCOMMON/SPEC/inference.md` §9 tells a consumer to read — reading it off the post-carry fan is correct, and `T1DMDROID`'s `RollingForecaster.terminalOffsets` does exactly that. What would double-count is reading it off the post-carry fan and then folding it into the carry AGAIN.
...
Markdown drift is a bug. Prefer editing sections over appending. Delete stale paragraphs rather than leaving them. `README.md`, `ARCHITECTURE.md`, `docs/` and `docs/COMPARISON.md` are PUBLIC — no second person, no prescriptive prose, no ranked action items, no speculation about future work.
...
- `validate.py <checkpoint>` — `train.py`'s `_run_validation` and table, standalone, at the checkpoint's dims (`finetune._apply_checkpoint_dims`), policy (dispatching to `train_blind`) and stored normalization stats; without `--backup` the `val` patients `train.py` draws for that master seed, live or from `--cache-path`; with `--backup` the gap-free runs of the trailing `t1dmdroid_converter.TEST_DAYS`. `overfit_ratio` is simulator-only
...
- `risk_loss.py` — `risk_total_loss` (pinball + per-span DILATE mixed with MSE by `MSE_ALPHA` + Kendall-Gal combine; f-target applied once)
...
- `deploy_model.sh <checkpoint> [model-id]` — XNNPACK export under `.venv-export` into `exported/<id>/`, then `adb push` of the `.pte`, head file and descriptor into T1DMDROID's external-files `models/`; the id defaults to the capacity directory of a checkpoint under `models/<capacity>/checkpoints/` and is required for any other path, since a repeat push overwrites; the app discovers it on its next service start
...
- `model_health.py` — capacity audit of a checkpoint, keyed to `resize_model.py`'s knobs plus the context window; weight evidence always (optimizer staleness replayed over `_build_optimizers`' two Muon groups, drift, spectra, per-(layer, head) and per-(layer, unit) strength), and under `--data N` activations, per-head attention entropy / self-mass / lag reach, block influence and CKA, the spline's per-step departure, one-at-a-time ablation of every head / sublayer / block, a context-truncation ladder and keep-top-k width ladders, all scored by Δ pinball on the same windows; the data pass re-imports `model` and `data` against the checkpoint's dims, builds its windows on the `val` partition under the CHECKPOINT's `masked_channel_policy`, and the verdicts defer to the ladders where they exist
