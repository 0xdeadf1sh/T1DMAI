...
- `deploy_model.sh <checkpoint> [model-id]` — XNNPACK export under `.venv-export` into `exported/<id>/`, then `adb push` of the `.pte`, head file and descriptor into T1DMDROID's external-files `models/`; the id defaults to the capacity directory, so a repeat push overwrites; the app discovers it on its next service start
