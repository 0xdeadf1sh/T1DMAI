...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--ema-decay", type=float, default=None, help="default: config.EMA_DECAY"
    )
    p.add_argument("--validation-interval", type=int, default=1000)
    p.add_argument("--eval-windows", type=int, default=32768)
...
def main() -> None:
    ...
    from config import (
        ...
        PREDICTION_PATCHES,
        QUANTILE_LEVELS,
        TIME_PROBE_LABEL_SMOOTH_BINS,
        TIME_PROBE_LOSS_WEIGHT,
        TIME_PROBE_N_BINS,
    )
    ...
    if ckpt is not None:
        ...
        if checkpoint_masked_channel_policy(ckpt) != masked_channel_policy(blind=False):
            sys.exit(
                f"--checkpoint {args.checkpoint}: blind masked-channel policy "
        ...
        for name in CHANNEL_NAMES:
            ...
            if (
                s is None
                or not np.isfinite(s.get("mean", np.nan))
                or not np.isfinite(s.get("std", np.nan))
                or s["std"] <= 0.0
            ):
                sys.exit(
                    f"--checkpoint {args.checkpoint}: bad normalization "
    ...
    def pick(spec: str | None, what: str) -> list[str] | None:
        ...
        if bad:
            raise SystemExit(
                f"--{what}-dataset: unknown {bad}; cache has {all_sources}"
            )
        return want
    ...
    train_ds = FinetuneTrainDataset(
        ...
        diadata_frac=args.diadata_frac,
    ...
    print(
        f"cache: {len(cache.subjects)} train subjects "
    ...
    def summarize(cols: dict, source: str | None = None) -> dict:
        ...
        out["mean_dts_a_band"] = (
            float(np.mean(dts_a_band_vals)) if dts_a_band_vals else float("-inf")
        )
        return out
    ...
    def evaluate() -> dict:
        ...
        for batch in eval_loader:
            ...
            lo_mgdl = (
                kovatchev_f_inv(fan[..., band_lo_idx]).reshape(B, -1).cpu().numpy()
            )
            hi_mgdl = (
                kovatchev_f_inv(fan[..., band_hi_idx]).reshape(B, -1).cpu().numpy()
            )
            ...
            for m, hs in zip(HORIZON_MINUTES, HORIZON_STEPS):
                ...
                a[3].append(t[ok])
                a[4].append(src[ok])
        assert pos == len(windows), (
            f"eval loader covered {pos} of {len(windows)} windows"
        )
        model.train()
        cols = {
            m: tuple(np.concatenate(v) if v else np.empty(0) for v in acc[m])
            for m in HORIZON_MINUTES
        }
        out = summarize(cols)
    ...
    def print_table(metrics: dict, best: float, best_step: int) -> None:
        ...
        print(
            "  min    DTS-A%  DTS-A%b     RMSE    RMSEb   MARD%   MARD%b"
            "    cov%   width       n"
        )
        for m in HORIZON_MINUTES:
            r = metrics[m]
    ...
    def print_by_source(by: dict) -> None:
        ...
        print(
            f"  {'sub-dataset':<16}{'n':>7} "
            + " ".join("".join(f"{m:>{w}}" for m in HORIZON_MINUTES) for _ in groups)
        )
    ...
    muon_opt, adam_opt = _build_optimizers(
        model, weighting, args.muon_lr, args.adam_lr, MUON_MOMENTUM, ADAM_WEIGHT_DECAY
    )
    ...
    log.writerow(
        ["step", "loss", "mean_dts_a", "mean_dts_a_band"]
        + [
            f"{k}_{m}"
            for m in HORIZON_MINUTES
            for k in (
    ...
    for batch in train_loader:
        ...
        if time_pred is not None and "slot_hour" in batch:
            tod_ce = time_of_day_bin_ce(
                time_pred[valid_b],
                batch["slot_hour"].to(device)[valid_b],
                TIME_PROBE_N_BINS,
                TIME_PROBE_LABEL_SMOOTH_BINS,
            )
        ...
        if skipped_reason is not None:
            ...
            adam_opt.zero_grad(set_to_none=True)
            print(
                f"step {step}: {skipped_reason}, step skipped ({n_skipped} total)",
                flush=True,
            )
        ...
        if step % args.log_interval == 0:
            ...
            t_last = time.time()
            print(
                f"step {step}/{args.total_steps}  loss {loss.item():.4f}  "
                f"Q {comps['loss_Q'].item():.4f}  D {comps['loss_D'].item():.4f}  "
                f"R {comps['loss_M'].item():.4f}  "
                f"{dt / args.log_interval:.2f}s/step",
                flush=True,
            )
    ...
    log_f.close()
    print(
        f"done. best mean DTS-A {best:.3f} @ step {best_step} "
        f"-> {os.path.join(args.out_dir, 'finetune_best.pt')}",
        flush=True,
    )
