"""Proper scoring rules for the masked-BG quantile fan, binned on ``d``: crps_by_d, winkler_by_d,
coverage_sharpness_by_d, joint_coverage_by_d, alarm_operating_curve. Space: mg/dL PHYSICAL only,
every entry runs ``_assert_mgdl``. ``d`` = patches to nearest visible evidence, the sole bin axis.
Pooled figures carry ``POOLED_NOT_COMPARABLE``, not a selection metric. No gates: numbers only.
"""
...
def _assert_mgdl(a: np.ndarray, name: str) -> None:
    ...
    Risk space spans ``[f(10), f(400)] = [-6.82, +3.16]``, z-space narrower — both miss the
    ``BG_CLAMP_MIN`` floor by a wide margin. Non-finite entries pass here, dropped by each rule.
    """
...
def _check_fan(q: np.ndarray, true: np.ndarray, d: np.ndarray,
    ...
    ``d`` must be >= 1: a padded head slot carries ``d = 0``, so a zero means an unfiltered
    ``(B, M)`` flatten reached the scorer and padded slots would score against patch 0.
    """
...
def central_levels(levels: Sequence[float] = QUANTILE_LEVELS) -> tuple[tuple[float, int, int], ...]:
    """Nominal central levels the fan's τ pairs imply, widest first -> (nominal, lo_idx, hi_idx).
...
class CoverageSharpness:
    ...
    Both required at construction: no coverage without its sharpness. ``kind`` names WHICH
    coverage — marginal per step or simultaneous; ``n`` counts that kind's own units: (unit,
    step) pairs for marginal, groups for joint."""
    kind: str
    nominal: float
    coverage: float
    mean_width: float
...
class JointCoverage:
    ...
    ``marginal_*`` is per (unit, step); ``joint_*`` is per GROUP (one origin), needing EVERY step
    in scope at once: ``joint_within_d`` = group's patches at that ``d``; ``joint_path_to_d`` =
    every step at ``d' <= d``; ``joint_pooled`` = every scored step. Joint coverage <= smallest
    marginal in scope — reporting one as the other overstates calibration."""
    nominal: float
...
class AlarmPoint:
    ...
    ``median_lead_min``: median minutes, decision time to first true sub-threshold step caught.
    A detection rate bought at a two-minute lead isn't usable; the rate alone can't show it."""
    rule: str
...
def crps_steps(q: np.ndarray, true: np.ndarray,
    ...
    """Per-(unit, step) CRPS from the discrete fan -> ``(N, S)`` mg/dL; non-finite ``true`` -> NaN.
    CRPS(F,y) = 2∫₀¹ ρ_τ(y−F⁻¹(τ))dτ; F⁻¹ has ``K=7`` nodes, tails shared closed-form between rules.
    ``CRPS_PWL`` (default): F⁻¹ linear between nodes, quadrature error ZERO — only fan's own
    discretization remains. ``CRPS_TRAPEZOID`` overstates near the crossing, worst case ≤ D/24 ≈
    0.0417·D mg/dL per gap D (20 mg/dL half-IQR costs up to 0.83 mg/dL)."""
...
def crps_by_d(q: np.ndarray, true: np.ndarray, d: np.ndarray,
    ...
    """CRPS averaged per ``d``, and pooled; mg/dL in/out. ``crps_steps`` carries quadrature error.

    Every step of a masked patch shares that patch's ``d``: a per-``d`` figure means over its (unit,
    step) pairs. Pooled averages the protocol's mask distribution: ``POOLED_NOT_COMPARABLE``."""
...
def winkler_steps(q: np.ndarray, true: np.ndarray, lo_idx: int, hi_idx: int,
    ...
    for ``[l, u]`` at nominal ``1 − α``. Proper: width alone is minimized by a collapsed interval,
    escape alone by an infinite one, so coverage and sharpness are priced on one axis."""
...
def winkler_by_d(q: np.ndarray, true: np.ndarray, d: np.ndarray,
    ...
    Levels come from ``central_levels``: 0.90, 0.80, 0.50 under ``QUANTILE_LEVELS``, key the result.
    """
    q, true, d, lv = _check_fan(q, true, d, levels)
    out: dict[float, ByD] = {}
...
def coverage_sharpness_by_d(q: np.ndarray, true: np.ndarray, d: np.ndarray,
    ...
    """Marginal per-step coverage with the mg/dL width that bought it, one ``CoverageSharpness``.

    Both fields required: no coverage without its width, a band widens to any coverage. MARGINAL =
    fraction of individual (unit, step) pairs inside the band; ``joint_coverage_by_d`` carries the
    simultaneous variant — the two are not interchangeable."""
    q, true, d, lv = _check_fan(q, true, d, levels)
...
def _joint(kind: str, nominal: float, inside: np.ndarray, width: np.ndarray,
    ...
    A group counts only if it carries one selected finite step; a non-finite step is excluded,
    never read as an escape."""
...
def joint_coverage_by_d(q: np.ndarray, true: np.ndarray, d: np.ndarray,
    ...
    """Simultaneous horizon coverage, apart from the per-step marginal figure; widths mg/dL.
    ``group`` labels the forecast origin each masked patch belongs to. Marginal asks whether one
    step landed inside its band; joint asks whether a whole path did at once. Joint is bounded by
    the smallest marginal in its scope and decays with scope: within one ``d``, cumulative out to
    a ``d``, or over the group's steps."""
    q, true, d, lv = _check_fan(q, true, d, levels)
...
def predictive_cdf(q: np.ndarray, x: float,
    ...
    """``P(Y <= x)`` under the PWL-quantile law ``CRPS_PWL`` scores; ``q``, ``x`` mg/dL.

    F⁻¹ linear between nodes, flat outside: mass below the lowest node is an atom, exactly 0 below
    ``q₁``, 1 at/above ``q_K``. The whole fan gives the alarm a continuous score to sweep."""
    q = np.asarray(q, dtype=np.float64)
    lv = np.asarray(levels, dtype=np.float64)
...
def forecast_lead_minutes(d: np.ndarray, n_steps: int = PATCH_SIZE) -> np.ndarray:
    ...
    Patch at distance ``d`` is the ``d``-th after origin: step ``s`` lands at
    ``((d − 1)·n_steps + s + 1)·GRID_MIN`` min, patch-end convention of ``metrics.core.horizons``.
    ``d = 1..4 ⇔ @30/@60/@90/@120`` min holds only here — for infill, ``d`` says nothing about time
    to origin, so that protocol passes its own ``lead_min``."""
    d = np.asarray(d)
...
def _alarm_curve_for(d_label: int | None, sel_units: np.ndarray, score_unit: np.ndarray,
    ...
    Per group: score = max over scanned steps, deployed band edge = min, event onset = earliest
    scanned step with ``true < threshold``."""
...
def alarm_operating_curve(q: np.ndarray, true: np.ndarray, d: np.ndarray,
    ...
    ``q``, ``true``, ``threshold`` mg/dL; score is a probability, lead minutes; one decision per
    ``group`` over its scanned steps. SCORE = max ``P(BG<=threshold)``; EVENT = ``true<threshold``
    step (strict), ONSET = earliest; DETECTION RATE = detected/events; FALSE ALARMS/DAY = firing
    groups w/o event / ``observed_days`` (None -> count only); MEDIAN LEAD = median ``lead_min``."""
    q, true, d, lv = _check_fan(q, true, d, levels)
    group = _check_group(group, d.size)
...
def score_fan(q: np.ndarray, true: np.ndarray, d: np.ndarray, group: np.ndarray,
    ...
    ``lead_min=None`` skips the alarm curve: a detection rate without a lead time is not an
    operating point. No figure returned here is gated."""
    return FanScores(
